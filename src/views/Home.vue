<template>
  <div ref="first" class="projects third-body">
    <Project
      ...
      :classes="[
        'auth',
        $data.selectedNodes.find((a) => a.side === 0 && a.index === index)
          ? 'selected'
          : undefined,
      ]"
      :id="'source' + index"
      :disabled="
        $data.changingConnection &&
        (source.type === 'sink-input' || $data.connectionType === 'sink')
      "
      :action="
        function () {
          if (!$data.changingConnection) {
            $data.nodeSelection =
              $data.nodeSelection === undefined
                ? { side: 0, index }
                : undefined;
            ...
            if (
              ...
              $refs.createVirtualCableModal.showModal();
          }
        }
      "
      :title="
  ...
  <div ref="second" class="projects third-body right">
    <Project
      ...
      :classes="[
        'auth',
        $data.selectedNodes.find((a) => a.side === 1 && a.index === index)
          ? 'selected'
          : undefined,
      ]"
      :id="'sink' + index"
      ...
      :disabled="
        $data.changingConnection &&
        (sink.type === 'source-output' || $data.connectionType === 'source')
      "
      :action="
        function () {
          if (!$data.changingConnection) {
      ...
      <div class="modal_left cable_modal_div">
        ...
        {{
          $data.currentFocusedCable.type === "virtual-cable"
            ? $data.sources.find(
                (source) =>
                  $data.currentFocusedCable.output.source === source.id
              ).description
      ...
      <div class="modal_right cable_modal_div">
        ...
        {{
          this.$data.currentFocusedCable.type === "virtual-cable"
            ...
            : $data.sinks.find(
                (sink) => sink.id === $data.currentFocusedCable.id
              ).description
            ? $data.sinks.find(
                (sink) => sink.id === $data.currentFocusedCable.id
              ).description
  ...
  <Modal
    ...
    :buttons="[
      {
        title: 'Disregard all changes made.',
        text: 'Cancel',
        action: function () {
          $refs.manageNodeModal.hideModal();
          $data.nodeSelection = undefined;
        },
      },
    ]"
  >
    <template v-if="$data.nodeSelection !== undefined">
      {{
        $data.nodeSelection.side === 0
          ? $data.sources[$data.nodeSelection.index].description
          : $data.sinks[$data.nodeSelection.index].description
      }}<br />
      <button
        class="button"
        @click="beginCreateVirtualCable"
        :disabled="($data.nodeSelection.side === 0
          ? $data.sources[$data.nodeSelection.index].type
          : $data.sinks[$data.nodeSelection.index].type) === 'sink-input' || ($data.nodeSelection.side === 0
          ? $data.sources[$data.nodeSelection.index].type
          : $data.sinks[$data.nodeSelection.index].type) === 'source-output'"
        title="Create a virtual cable using this node!"
      >
        Create Virtual Cable</button
      ><br /><br />
      <p
        v-if="
          ($data.nodeSelection.side === 0
            ? $data.sources[$data.nodeSelection.index].driver
            : $data.sinks[$data.nodeSelection.index].driver) ===
          'module-null-sink.c'
        "
      >
        If you delete a virtual node with active connections to it, those
        connections will move to other available nodes and might create a
        feedback loop. Be careful!
      </p>
      <button
        class="button"
        v-if="
          ...
          'module-null-sink.c'
        "
        title="This will delete this null node. Any connections to it will be moved to possibly unwanted locations."
        @click="this.deleteNullSink"
      >
        Delete Virtual Node
      </button>
    </template>
  ...
  <Modal
    title="Create Virtual Cable"
    ref="createVirtualCableModal"
    :buttons="[
      {
  ...
  >
    ...
    {{
      $data.selectedNodes[0].side === 0
        ? $data.sources[$data.selectedNodes[0].index].description
        : $data.sinks[$data.selectedNodes[0].index].description
    }}
    to
    {{
      $data.selectedNodes[1].side === 0
        ? $data.sources[$data.selectedNodes[1].index].description
        : $data.sinks[$data.selectedNodes[1].index].description
    }}?
  </Modal>
  <Modal
    title="Change Connection"
    ref="changeConnectionModal"
    :buttons="[
      {
        title: 'Cancel the operation.',
        text: 'Cancel',
  ...
  >
    Are you sure you want to change the connection to
    {{
      $data.selectedNodes[0].side === 0
        ? $data.sources[$data.selectedNodes[0].index].description
        : $data.sinks[$data.selectedNodes[0].index].description
...
export default class Home extends Vue {
  ...
  deleteNullSink() {
    ...
    if (this.$data.nodeSelection?.side === 0)
      sink = this.$data.sinks.find(
        (a) =>
          this.$data.sources[this.$data.nodeSelection?.index as number]
            .owner === a.owner
      ) as PASink;
    else
      sink = this.$data.sinks[
        this.$data.nodeSelection?.index as number
      ] as PASink;
  ...
  async calculateChanges() {
    ...
    let mostRecentSinks: (PASink | PASourceOutput)[] = [
      ...((await pactlUtils.fetchFromPactl("sinks")) as PASink[]),
      ...((await pactlUtils.fetchFromPactl(
        "source-outputs"
      )) as PASourceOutput[]),
    ];
    let newSinks: (PASink | PASourceOutput)[] = mostRecentSinks.filter(
      (a) => !this.$data.sinks.find((b) => b.id === a.id && b.type === a.type)
    );
    oldSinks = this.$data.sinks.filter(
      (a) => !mostRecentSinks.find((b) => a.id === b.id && a.type === b.type)
    );

    let mostRecentSources: (PASource | PASinkInput)[] = [
      ...((await pactlUtils.fetchFromPactl("sources")) as PASource[]),
      ...((await pactlUtils.fetchFromPactl("sink-inputs")) as PASinkInput[]),
    ];
    let newSources: (PASource | PASinkInput)[] = mostRecentSources.filter(
      (a) => !this.$data.sources.find((b) => b.id === a.id && b.type === a.type)
    );
    oldSources = this.$data.sources.filter(
      (a) => !mostRecentSources.find((b) => a.id === b.id && a.type === b.type)
    );

    let mostRecentVirtualCables: PAVirtualCable[] =
      await pactlUtils.getVirtualCables();
    let newVirtualCables: PAVirtualCable[] = mostRecentVirtualCables.filter(
      (a) =>
        !this.$data.virtualCables.find(
          (b) =>
            b.output.type === a.output.type &&
            b.output.id === a.output.id &&
            b.input.type === a.input.type &&
            b.input.id === a.input.id
        )
    );
    oldVirtualCables = this.$data.virtualCables.filter(
      (a) =>
        !mostRecentVirtualCables.find(
          (b) =>
            a.output.type === b.output.type &&
            a.output.id === b.output.id &&
            a.input.type === b.input.type &&
            a.input.id === b.input.id
        )
    );
    ...
    if (
      ...
      newSources.length > 0 ||
      oldSources.length > 0 ||
      newVirtualCables.length > 0 ||
      oldVirtualCables.length > 0
    )
      this.updateData();
  }
  ...
  async createVirtualCable() {
    await pactlUtils.createVirtualCable(
      this.$data.sources[
        (this.$data.selectedNodes.find((a) => a.side === 0) as NodeInfo).index
      ] as PASource,
      this.$data.sinks[
        (this.$data.selectedNodes.find((b) => b.side === 1) as NodeInfo).index
      ] as PASink
    );
  ...
  async changeConnection() {
    if (this.$data.currentFocusedCable instanceof PASinkInput) {
      await this.$data.currentFocusedCable.changeSink(
        this.$data.sinks[this.$data.selectedNodes[0].index] as PASink
      );
    }
    if (this.$data.currentFocusedCable instanceof PASourceOutput) {
      await this.$data.currentFocusedCable.changeSource(
        this.$data.sources[this.$data.selectedNodes[0].index] as PASource
      );
    }
    if (this.$data.currentFocusedCable instanceof PAVirtualCable) {
      if (this.$data.connectionType === "sink") {
        await this.$data.currentFocusedCable.input.changeSink(
          this.$data.sinks[this.$data.selectedNodes[0].index] as PASink
        );
      } else if (this.$data.connectionType === "source") {
        await this.$data.currentFocusedCable.output.changeSource(
          this.$data.sources[this.$data.selectedNodes[0].index] as PASource
        );
      }
    }
